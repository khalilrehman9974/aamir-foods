                                                        ...
                                                        <div class="col-lg-6 col-12 ">
                                                            <form
                                                                ...
                                                                <div class="form-group input-group">
                                                                    <div class="col-lg-0 col-12">
                                                                        ...
                                                                        <select id="main-head" name="main_head"
                                                                            ...
                                                                            @foreach($mainHeads as $index=>$value)
                                                                                <option
                                                                                    value="{{ $index }}" {{ (old("main_head") == $index ? "selected":"") || (!empty($subSubHead->main_head) ? collect($subSubHead->main_head)->contains($index) : '') ? 'selected':'' }}>{{ $value }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    ...
                                                                    <br>
                                                                    <div class="col-lg-0 col-12">
                                                                        <label for="inputState" class="form-label">Control Head</label>
                                                                        @if(!empty($subSubHead))
                                                                            <select id="control-head"
                                                                                ...
                                                                                @foreach($controlHead as $key=>$value)
                                                                                    <option
                                                                                        value="{{ $key }}" {{!empty($subSubHead) && ($subSubHead->control_head == $key) ? "selected" : ''}} {{($key == old('control_head')) ? 'selected' : ''}}>{{ $value }}</option>
                                                                                @endforeach
                                                                            </select>
                                                                        ...
                                                                        @if(!empty($subSubHead))
                                                                            <select id="sub-head"
                                                                                ...
                                                                                @foreach($subHeads as $key=>$value)
                                                                                    <option
                                                                                        value="{{ $key }}" {{!empty($subSubHead) && ($subSubHead->sub_head == $key) ? "selected" : ''}} {{($key == old('sub')) ? 'selected' : ''}}>{{ $value }}</option>
                                                                                @endforeach
                                                                            </select>
                                                                    ...
                                                                    <a href="{{ route('detail-account.list') }}"
                                                                       class="btn btn-dark rounded bs-popover ml-2 mt-5  mb-4">Cancel</a>                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </div>
