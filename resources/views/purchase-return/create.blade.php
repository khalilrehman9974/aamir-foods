    ...
    <script>
        document.getElementsByClassName('additem')[0].addEventListener('click', function() {
            ...
            let $html = '<tr>' +
                ...
                '</ul>' +
                '</td>' +
                '<td class="description">    <select id="product_id" type="text" name="product_id[]" class="form-control select2 {{ config('constants.css-classes.ELEMENT_SIZE_CLASS') }} mb-3 custom-select"> <option value=""> Select</option> @foreach ($dropDownData['products'] as $key => $value) <option value="{{ $key }}" {{ (old('product_id') == $key ? 'selected' : '') || (!empty($purchase->product_id) ? collect($purchase->product_id)->contains($key) : '') ? 'selected' : '' }}> {{ $value }} </option> @endforeach </select> ' +
                '<td class="rate">' +
                '<input type="text" class="form-control  {{ config('constants.css-classes.ELEMENT_SIZE_CLASS') }}" placeholder="Qty">' +
                ' </td>' +
                '<td class="text-right qty"><input type="text" class="form-control  {{ config('constants.css-classes.ELEMENT_SIZE_CLASS') }}" placeholder="unit"></td>' +
