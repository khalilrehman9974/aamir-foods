    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('crv.list') }}">List of Cash Receipt Vouchers</a></li>
                <li class="breadcrumb-item"><a href="{{ route('crv.create') }}">Create Voucher</a></li>
                ...
                <div class="widget-header">
                    <div class="row">
                        <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                            <h4>Create Cash Receipt Voucher</h4>
                        </div>
                    </div>
                </div>
                                                ...
                                                <div class="col-lg-12 col-12 ">
                                                    <form
                                                        ...
                                                        <div class="form-group">
                                                                        ...
                                                                        <table class="table item-table">
                                                                            <thead>
                                                                                <tr>
                                                                                    ...
                                                                                    <th class="" style="width: 10%;">Code</th>
                                                                                ...
                                                                                <tr>
                                                                                    ...
                                                                                    <td class="title">
                                                                                        <select id="account_title_id"
                                                                                            ...
                                                                                            @foreach ($dropDownData['accounts'] as $key => $value)
                                                                                                <option
                                                                                                    value="{{ $key }}"
                                                                                                    {{ (old('account_title_id') == $key ? 'selected' : '') || (!empty($crv->account_title_id) ? collect($crv->account_title_id)->contains($key) : '') ? 'selected' : '' }}>
                                                                                                    {{ $value }}
                                                                                                </option>
                                                                                            @endforeach
                                                                                        </select>
                                                                                    ...
                                                                                    <td class="title">
                                                                                        <textarea id="unit" type="text" name="description[]"
                                                                                            value="{{ old('description', !empty($crv->description) ? $crv->description : '') }}"
                                                                                    ...
                                                                                    <td class="text-right title">
                                                                                        <input id="debit"
                                                                                            type="text"
                                                                                            name="debit[]"
                                                                                            value="{{ old('debit', !empty($crv->debit) ? $crv->debit : '') }}"
                                                                                            placeholder="Amount "
                                                                                            class="form-control form-control-sm">
                                                                                    </td>
                                                            ...
                                                            <a href="{{ route('crv.list') }}" style="float: right;"
    ...
    <script>
        document.getElementsByClassName('additem')[0].addEventListener('click', function() {
            ...
            let $html = '<tr>' +
                ...
                '<td class="title"><select id="account_title_id" name = "account_title_id[]" class = "form-select form-control-sm" > <option selected = "" >Please select the Party </option> @foreach ($dropDownData['accounts'] as $key => $value) <option value = "{{ $key }}" {{ (old('account_title_id') == $key ? 'selected' : '') || (!empty($crv->account_title_id) ? collect($crv->account_title_id)->contains($key) : '') ? 'selected' : '' }}>{{ $value }} </option> @endforeach </select> </td>' +
                '<td class="title"><textarea id="description" type="text" name="description[]" value="{{ old('description', !empty($crv->description) ? $crv->description : '') }}" placeholder="Please Enter Description " class="form-control form-control-sm"></textarea></td>' +
                '<td class="text-right title"> <input id="debit" type="text" name="debit[]" value="{{ old('debit', !empty($crv->debit) ? $crv->debit : '') }}" placeholder="Amount " class="form-control form-control-sm"></td>' +
