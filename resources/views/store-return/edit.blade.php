<x-base-layout :scrollspy="false">
            ...
            <div class="statbox widget box box-shadow">
                ...
                <div class="col-md-12 mb-4">
                    <div class="widget-content widget-content-area">
                        <form method="POST"
                            action="{{ !empty($storeReturnMaster) ? route('storeReturn.update') : route('storeReturn.save') }}"
                            class="row g-3 needs-validation" novalidate>
                            @csrf
                            <input type="hidden" name="id" id="id"
                                value="{{ isset($storeReturnMaster->id) ? $storeReturnMaster->id : '' }}" />
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-md-6">
                                        <label for="issued_to">Issue Note#</label>
                                        <input type="text" value="{{ $storeReturnMaster->id }}" style="color: black;"
                                    ...
                                    {{-- <div class="col-md-6">
                                        ...
                                        <select id="product_id" name="product_id"
                                            ...
                                            @foreach ($dropDownData['products'] as $key => $value)
                                                <option value="{{ $key }}"
                                                    {{ (old('product_id') == $key ? 'selected' : '') || (!empty($storeReturnMaster->product_id) ? collect($storeReturnMaster->product_id)->contains($key) : '') ? 'selected' : '' }}>
                                                    {{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div> --}}
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text"
                                            value="{{ old('receiver_name', @$storeReturnMaster->receiver_name) }}"
                                            name="receiver_name"
                                            class="form-control {{ config('constants.css-classes.ELEMENT_SIZE_CLASS') }}"
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select id="from_department" name="from_department"
                                            ...
                                            @foreach ($dropDownData['departments'] as $key => $value)
                                                <option value="{{ $key }}"
                                                    {{ (old('from_department') == $key ? 'selected' : '') || (!empty($storeReturnMaster->from_department) ? collect($storeReturnMaster->from_department)->contains($key) : '') ? 'selected' : '' }}>
                                                    {{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-6">
                                        ...
                                        <select id="to_department" name="to_department"
                                            ...
                                            @foreach ($dropDownData['departments'] as $key => $value)
                                                <option value="{{ $key }}"
                                                    {{ (old('to_department') == $key ? 'selected' : '') || (!empty($storeReturnMaster->to_department) ? collect($storeReturnMaster->to_department)->contains($key) : '') ? 'selected' : '' }}>
                                                    {{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="invoice-detail-items " style="padding: 0px 0px 0px 0px !important;">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table item-table">
                                                ...
                                                <tbody>
                                                    @foreach ($storeReturnDetails as $storeReturnDetail)
                                                        @php
                                                        ...
                                                        <tr class="tr_clone validator_{{ $index }}">
                                                            ...
                                                            <td class="product">
                                                                <select id="product_id" name="product_id[]"
                                                                    ...
                                                                    @foreach ($dropDownData['products'] as $key => $value)
                                                                        <option value="{{ $key }}"
                                                                            {{ (old('product_id') == $key ? 'selected' : '') || (!empty($storeReturnDetail->product_id) ? collect($storeReturnDetail->product_id)->contains($key) : '') ? 'selected' : '' }}>
                                                                            {{ $value }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <td class="quantity">
                                                                <input type="text" style="color: black;"
                                                                    ...
                                                                    placeholder="P.T"
                                                                    value="{{ old('packing_type', !empty($storeReturnDetail->packing_type) ? $storeReturnDetail->packing_type : '') }}"
                                                                    readonly>
                                                            </td>
                                                            <td class="quantity">
                                                                <input type="text" style="color: black; "
                                                                    ...
                                                                    value="{{ old('measurement_type', !empty($storeReturnDetail->measurement_type) ? $storeReturnDetail->measurement_type : '') }}"
                                                            ...
                                                            <td class="quantity">
                                                                <input type="text" style="color: black;"
                                                                    ...
                                                                    value="{{ old('size', !empty(@$storeReturnDetail->size) ? @$storeReturnDetail->size : '') }}"
                                                                    readonly>
                                                            </td>

                                                            <td class="bagsQuantity">
                                                                <input type="number" id="bags" name="bags[]"
                                                                    value="{{ old('bags', !empty(@$storeReturnDetail->bags) ? @$storeReturnDetail->bags : '') }}"
                                                                    style="color: black;"
                                                            ...
                                                            <td class="avgWeight">
                                                                <input type="number" id="avgWeight"
                                                                    ...
                                                                    value="{{ old('avg_weight', !empty(@$storeReturnDetail->avg_weight) ? @$storeReturnDetail->avg_weight : '') }}"
                                                            ...
                                                            <td class="totQuantity">
                                                                <input type="number" id="totalQty"
                                                                    name="total_qty[]"
                                                                    value="{{ old('total_qty', !empty(@$storeReturnDetail->total_qty) ? @$storeReturnDetail->total_qty : '') }}"
                                                            ...
                                                            <td class="quantity">
                                                                <textarea id="unit" type="text" name="remarks[]" placeholder="Remarks...."
                                                                    class="mt-0 form-control {{ config('constants.css-classes.ELEMENT_SIZE_CLASS') }} remarks_{{ $index }}">{{ @$storeReturnDetail->remarks }}</textarea>
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
    ...
    <script>
        document.getElementsByClassName('additem')[0].addEventListener('click', function() {
            ...
            let $html = '<tr>' +
                ...
                currentIndex +
                '"><option selected =""> Please select the product </option> @foreach ($dropDownData['products'] as $key => $value) <option value = "{{ $key }}"{{ (old('product_id') == $key ? 'selected' : '') || (!empty($storeReturnDetail->product_id) ? collect($storeReturnDetail->product_id)->contains($key) : '') ? 'selected' : '' }}> {{ $value }}</option> @endforeach < /select> ' +
                '</td>' +
                '<td class="quantity">' +
    ...
    <script>
        var config = {
            routes: {
                getProductPackingTypeDetail: "{{ url('storeReturn/get-product-packing-type') }}",
                getProductMeasurementTypeDetail: "{{ url('storeReturn/get-product-measurement-type') }}",
                getProductSizeDetail: "{{ url('storeReturn/get-product-size') }}",
            },
        }
    </script>
